	...
	for(var i = 0; i < 12; i++){
		for(var j = 0; j < 12 ; j++){
			...
			if( Mat[i][j] === 0 ){
				...
				rbga_color = 'rgba('+(243).toString()+','+(132).toString()+','+(41).toString()+','+(0.6 - Mat[i][j]*.05 ).toString()+')'
			}
	...
	for(var i = 0; i < 12; i++){
		for(var j = 0; j < 12 ; j++){
			...
			if( Mat[i][j] === 0 ){
				...
				rbga_color = 'rgba('+(127).toString()+','+(223).toString()+','+(111).toString()+','+(0.2 + Mat[i][j]*.05 ).toString()+')'
			}
			document.getElementById(stat_tag).style.background = rbga_color
		}
	}
