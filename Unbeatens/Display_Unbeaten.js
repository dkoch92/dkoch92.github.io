	...
	for(var i = 0; i < 12; i++){
		for(var j = 0; j < 12 ; j++){
			...
			if( Mat[i][j] === 0 ){
				...
				rbga_color = 'rgba('+(243).toString()+','+(132).toString()+','+(41).toString()+','+( .6 - 0.05*Mat[i][j]).toString()+')'
			}
...
function B_DisplayColors(Mat,id){
	for(var i = 0; i < 12; i++){
		for(var j = 0; j < 12 ; j++){
			stat_tag = id[i][j]
			if( Mat[i][j] > 0 ){
				rbga_color = 'rgba('+(127).toString()+','+(223).toString()+','+(111).toString()+','+( .01 + 0.05*Mat[i][j] ).toString()+')'
			}else{
